...
const controller = {
    writeHead: async (request, response) => {
        const test = response.writeHead(302, { "Location": "/src/photos/john123/profile.jpeg" });
    ...
    const parsedDB = JSON.parse(db); // array? object?
    
    let body = "";

    parsedDB.forEach(parsedDB => {
        //let imgPath = path.join(__diranme, `photos/${parsedDB.username}/${parsedDB.profile}`);
        body += `

        <div>
        <a>${parsedDB.username}</a>
        <img src="/src/photos/${parsedDB.username}/${parsedDB.profile}" alt="profile img" width="150" height="150">
        ...
        <!-- 정사각형 버튼 2 -->
        <button id="goToFeed">${parsedDB.username}</button>
        </div>
        `
    })
